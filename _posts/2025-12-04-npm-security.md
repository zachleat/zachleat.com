...
1. **Restrict npm Publishing Access** in the Settings tab for each npm package. Use _Require two-factor authentication and disallow tokens (recommended)_. Death to tokens!
1. Check in your {% icon "fas:lock" %}**lock file** (e.g. `package-lock.json` for npm). This is something I’ve personally felt a bit of resistance to, mostly because I hated managing git conflicts in these files (but dependabot has helped there). It is especially important when using a release script that uses npm packages to generate release artifacts. Prefer [`npm ci`](https://docs.npmjs.com/cli/v10/commands/npm-ci) over `npm install` in your release script.
